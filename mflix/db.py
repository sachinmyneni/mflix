...
def get_movies_by_country(countries):
    ...
    try:
        ...
        """
        # cursor = movies.find( { "countries": countries }, { "title": 1, "_id": 1 } )
        ...
        print(f"Exception in get_movies_by_country: {e}")
        return e
...
def add_user(name, email, hashedpw):
    ...
    try:
        ...
        db.users1 = db.users.with_options(write_concern=WriteConcern("majority"))
        db.users1.insert_one({
            "name": name,
            "email": email,
            "password": hashedpw
        })
...
def login_user(email, jwt):
    ...
    try:
        ...
        , upsert=True)
        if upsert_result.raw_result['ok'] == 1.0:
            return {"success": True}
        else:
...
def logout_user(email):
    ...
    try:
        ...
        delOne_result = db.sessions.delete_one({ "user_id": email })
        # print(dumps(delone_result,indent=2))
        return {"success": True}
    except Exception as e:
        return {"error": e}
